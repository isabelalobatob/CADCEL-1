...
export function resetZoomEvents(s, plt) {
    detachZoomEvents(s);
    const /** @type {?} */ unRegs = s._zoom.unRegs;
    const /** @type {?} */ evtOpts = { passive: s._touchEvents.start === 'touchstart', zone: false };
    const /** @type {?} */ slides = s._slides;
    let /** @type {?} */ slide;
    // Scale image
    if (s._supportGestures) {
    ...
    // Move image
    var /** @type {?} */ touchStartSub = s.ionSlideTouchStart.subscribe((ev) => {
    ...
    var /** @type {?} */ touchEndSub = s.ionSlideTouchEnd.subscribe(() => {
        onTouchEnd(s);
    });
    unRegs.push(() => { touchEndSub.unsubscribe(); });
    // Scale Out
    var /** @type {?} */ transEndSub = s.ionSlideTouchEnd.subscribe(() => {
        onTransitionEnd(s);
    });
    unRegs.push(() => { transEndSub.unsubscribe(); });
    if (s.zoomToggle) {
        var /** @type {?} */ doubleTapSub = s.ionSlideDoubleTap.subscribe(() => {
            toggleZoom(s, plt);
        });
